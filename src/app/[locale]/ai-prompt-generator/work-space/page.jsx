...
export default function WorkSpacePage() {
  ...
  const saveResult = (resultToSave = result) => {
    ...
    const isDuplicate = savedResults.some(
      (saved) => saved.optimizedPrompt === resultToSave.optimizedPrompt
    );

    if (isDuplicate) {
      toast.info(t('errors.alreadySaved'));
      return;
    }
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 p-2 md:p-4 lg:h-[calc(100vh-67px)]">
      {/* Left Column: Input Panel */}
      <div className="w-full lg:w-2/5 lg:h-full lg:overflow-y-auto pr-2 space-y-6">
        <Card className="h-full flex flex-col">
          ...
          <CardContent className="space-y-4 flex-grow flex flex-col">
            ...
            <div className="space-y-2 flex-grow flex flex-col">
              <Textarea id="prompt-input"  placeholder={t('input.placeholder')} value={prompt} onChange={(e) => { setPrompt(e.target.value); if (validationErrors.prompt) setValidationErrors(p => ({ ...p, prompt: null })); }} onFocus={() => { trackFieldInteraction('prompt_input', 'focus'); if (!prompt) trackFormStart(); }} onBlur={() => trackFieldInteraction('prompt_input', 'blur')} className={`text-base !field-sizing-fixed flex-grow resize-none ${validationErrors.prompt ? 'border-red-500' : ''}`} maxLength={2000} disabled={isLoading} aria-label={t('input.placeholder')} />
      ...
      <div className="w-full lg:w-3/5 lg:h-full lg:overflow-y-auto pl-2">
        {isLoading ? (
            ...
            <TabsContent value="result" className="mt-4 space-y-4">
              {result ? (<>
                <Card>
                  ...
                  <CardContent>
                    <div className={"relative bg-[#282c34] text-[0.875rem] text-[#bfc7d5] p-4 rounded-lg text-sm "}><Button size="icon" variant="ghost" className="absolute top-2 right-2 h-8 w-8" onClick={() => copyToClipboard(result.optimizedPrompt)}><Copy className="h-4 w-4" /></Button><pre className={"whitespace-pre-wrap " + jetBrainsMono.className}>{result.optimizedPrompt}</pre></div>
                ...
                {result.keyImprovements?.length > 0 && <Card><CardHeader><CardTitle>{t('result.improvements')}</CardTitle></CardHeader><CardContent><ul className="space-y-3">{result.keyImprovements.map((imp, i) => <li key={i} className="flex items-start gap-3 text-sm"><CheckCircle2 className="size-4 text-green-500 mt-0.5 flex-shrink-0" /><span>{imp}</span></li>)}</ul></CardContent></Card>}
