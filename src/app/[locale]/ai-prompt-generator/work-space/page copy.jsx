...
export default function WorkSpacePage() {
  ...
  useEffect(() => {
    ...
    if (saved) try { setSavedResults(JSON.parse(saved)); } catch (e) { console.error('Failed to load saved results:', e); }
    
    const history = localStorage.getItem('kari-optimization-history');
    if (history) try { setOptimizationHistory(JSON.parse(history)); } catch (e) { console.error('Failed to load optimization history:', e); }
  }, []);
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start p-4 md:p-6">
        ...
        {isLoading ? (
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            ...
            <TabsContent value="result" className="mt-4 space-y-4">
              <Card>
                <CardHeader><CardTitle className="flex items-center justify-between">{t('result.optimizedPrompt')}<div className="flex items-center gap-2"><Button variant="outline" size="sm" onClick={regeneratePrompt} disabled={isLoading}><RotateCcw className="w-4 h-4 mr-1" />{t('result.regenerate')}</Button><Button variant="outline" size="sm" onClick={() => { setResult(null); setError(null); }}><RefreshCw className="w-4 h-4 mr-1" />{t('result.newPrompt')}</Button></div></CardTitle></CardHeader>
                <CardContent>
                  <div className="relative bg-gray-900 text-white p-4 rounded-lg font-mono text-sm"><Button size="icon" variant="ghost" className="absolute top-2 right-2 h-8 w-8" onClick={() => copyToClipboard(result.optimizedPrompt)}><Copy className="h-4 w-4" /></Button><pre className="whitespace-pre-wrap">{result.optimizedPrompt}</pre></div>
                  <div className="flex items-center gap-2 mt-4"><Button onClick={() => saveResult()}><Save className="w-4 h-4 mr-1" />{t('result.save')}</Button><Button variant="outline" onClick={shareResult}><Share2 className="w-4 h-4 mr-1" />{t('result.share')}</Button></div>
                </CardContent>
              </Card>
              {result.keyImprovements?.length > 0 && <Card><CardHeader><CardTitle>{t('result.improvements')}</CardTitle></CardHeader><CardContent><ul className="space-y-3">{result.keyImprovements.map((imp, i) => <li key={i} className="flex items-start gap-3"><CheckCircle className="w-5 h-5 text-green-500 mt-0.5 flex-shrink-0" /><span>{imp}</span></li>)}</ul></CardContent></Card>}
